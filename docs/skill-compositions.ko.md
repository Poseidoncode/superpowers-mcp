...
### 메타 스킬: 세션 포렌식 (Session Forensics)

네 가지 파이프라인 밖에, **`diagnosing-superpowers`**는 디스크의 트랜스크립트에서 과거 세션의 문제 경위를 재구성합니다: 인테이크 인터뷰, 세션 탐색, 인용 증거가 있는 병렬 애널리스트 보고서, 그리고 선택적으로 스크럽된 번들이나 GitHub 이슈 초안. 세션이 계획을 무시했거나 작업을 반복했거나 설명할 수 없는 결과를 냈을 때 사용하고, 소견이 업스트림에 속하면 메인테이너 보고도 대신 작성합니다. MCP 서버는 스킬 내용 제공만 담당하며, 에이전트가 자체 도구로 호스트의 기록 파일을 읽기 때문에 트랜스크립트가 서버를 경유하지 않습니다.
