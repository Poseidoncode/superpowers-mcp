...
### メタスキル：セッションフォレンジック (Session Forensics)

4 つのパイプラインの外側で、**`diagnosing-superpowers`** はディスク上のトランスクリプトから過去セッションの問題経緯を再構成します：インテーク、セッション特定、証拠付きの並列アナリスト報告、そして任意でスクラブ済みバンドルや GitHub issue 草案。セッションが計画を無視した、作業を繰り返した、説明不能な結果を出した時に使い、所見が上流に属する場合はメンテナ向け報告も起草します。MCP サーバーはスキル内容の提供のみを担い、エージェントが自身のツールでホストの記録を読むため、トランスクリプトがサーバーを経由することはありません。
